...
# 定义正则模式
re_ref_patterns = {
    'Issue_PR': [r'https?://(?:www.)?github(?:-redirect.dependabot)?.com/[^\s/]+/[^\s/]+/(?:issues|pull)/\d+#\w+-\d+(?![\d/])',  # 所有Issue与PullRequest下的评论均为IssueCommentEvent事件
                 r'https?://(?:www.)?github(?:-redirect.dependabot)?.com/[^\s/]+/[^\s/]+/issues/\d+(?![\d/])', r'https?://(?:www.)?github(?:-redirect.dependabot)?.com/[^\s/]+/[^\s/]+/pull/\d+(?![\d/])',  # IssueEvent与PullRequestEvent事件是两个独立的事件
                 r'https?://(?:www.)?github(?:-redirect.dependabot)?.com/[^\s/]+/[^\s/]+/pull/\d+#discussion_r\d+(?![\d/])',  # PullRequestReveiwCommentEvent事件
                 r'[^\s\(\[\>\'\"\r\n;\\n]*#\d+(?![\d/])'],  # IssueEvent或PullRequestEvent事件
    'SHA': [r'https?://(?:www.)?github(?:-redirect.dependabot)?.com/[^\s/]+/[^\s/]+/pull/\d+/commits/[0-9a-fA-F]{40}(?![0-9a-fA-F])', r'https?://(?:www.)?github(?:-redirect.dependabot)?.com/[^\s/]+/[^\s/]+/commit/[0-9a-fA-F]{40}(?![0-9a-fA-F])', r'(?:(?<=[\s\(\[:])|(?<=^))([0-9a-f]{40})(?![0-9a-f])', r'(?:(?<=[\s\(\[:])|(?<=^))([0-9a-f]{7})(?![0-9a-f])'],
    ...
    'repo': [r'https?://(?:www.)?github(?:-redirect.dependabot)?.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-0-9a-zA-Z\.]*(?![-A-Za-z0-9\./])'],
    'github_src': [r'https?://(?:www.)?github(?:-redirect.dependabot)?.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-0-9a-zA-Z\.]*/files[-_A-Za-z0-9\.%#/]*(?![-_A-Za-z0-9\.%#/])', r'https?://(?:www.)?github(?:-redirect.dependabot)?.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-0-9a-zA-Z\.]*/pull/\d+/files[-_A-Za-z0-9\.%#/]*(?![-_A-Za-z0-9\.%#/])', r'https?://(?:www.)?github(?:-redirect.dependabot)?.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-0-9a-zA-Z\.]*/blob/[-_A-Za-z0-9\.%#/]*(?![-_A-Za-z0-9\.%#/])'],
    'github_other_links': [r'(https?://(?:www.)?github(?:-redirect.dependabot)?.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-0-9a-zA-Z\.]*/(?!issues|pull|commit|files|blob)[-_A-Za-z0-9\.%#/]*(?![-_A-Za-z0-9\.%#/]))'],
    'outer_http': [r'(https?://(?!www.github.com|github.com|github-redirect.dependabot.com)[-_A-Za-z0-9\.%#/]+(?![-_A-Za-z0-9\.%#/]))']
}

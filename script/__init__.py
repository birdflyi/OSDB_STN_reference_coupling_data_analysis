...
re_ref_patterns = {
    'Issue_PR': [r'https?://(?:www\.)?github(?:-redirect\.dependabot)?\.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-_0-9a-zA-Z\.]*/(?:issues|pull)/\d+#[-_0-9a-zA-Z\.%#/:]+\d+(?![\d/])',  # Issue与PullRequest下的事件，包含了IssueEvent（示例：issues/{issue_number}#issue-{issue_id}）或PullRequestEvent事件（示例：(?:issues|pull)/{issue_number}#issue-{issue_id}），IssueCommentEvent事件（Issue与PullRequest下的评论均为IssueCommentEvent事件，示例{_repo_full_name}/issues/{issue_number}#issuecomment-{issue_comment_id}），PullRequestReview事件（示例：{issue_number}#pullrequestreview-{pull_review_id}），PullRequestReviewCommentEvent事件的部分格式（示例：{issue_number}#discussion_r{pull_review_comment_id}）
                 ...
                 r'https?://(?:www\.)?github(?:-redirect\.dependabot)?\.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-_0-9a-zA-Z\.]*/(?:issues|pull)/\d+/files(?:/[0-9a-fA-F]{40})?#r\d+(?![\d#/])',  # PullRequestReveiwCommentEvent事件的部分格式（示例：{issue_number}/files#r{pull_review_comment_id} or {issue_number}/files/{push_head}#r{pull_review_comment_id}）
                 r'(?<!\\)(?:(?!\\r|\\n|\\t)[^\s\(\[\]\>\'\";]?)*#0*[1-9][0-9]*(?![\d/#a-z])'],  # IssueEvent或PullRequestEvent事件或未知的类型
    'SHA': [r'https?://(?:www\.)?github(?:-redirect\.dependabot)?\.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-_0-9a-zA-Z\.]*/pull/\d+/commits/[0-9a-fA-F]{40}(?![0-9a-fA-F/#])', r'https?://(?:www\.)?github(?:-redirect\.dependabot)?\.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-_0-9a-zA-Z\.]*/commit/[0-9a-fA-F]{40}(?![0-9a-fA-F/#])', r'(?:(?<=[\s\(\[:])|(?<=^))([0-9a-fA-F]{40})(?![0-9a-fA-F/#])', r'(?:(?<=[\s\(\[:])|(?<=^))([0-9a-fA-F]{7})(?![0-9a-fA-F/#])'],  # 包含Commit的sha，以及未知类型实体的sha
    ...
    'Repo': [r'https?://(?:www\.)?github(?:-redirect\.dependabot)?\.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-_0-9a-zA-Z\.]*(?![-_A-Za-z0-9\./])'],
    'Branch_Tag': [r'https?://(?:www\.)?github(?:-redirect\.dependabot)?\.com/[A-Za-z0-9][-0-9a-zA-Z]*/[A-Za-z0-9][-_0-9a-zA-Z\.]*/tree/[^\:\[\?\s\*\\\\,;\]`]+(?!>[\:\[\?\s\*,#\|\+@\<\>≤≥\=≠\&$\{\}\]\";`])'],
